...
class Text2GraphDataModule(pl.LightningDataModule):
    def __init__(
        ...
        word_mask=0.0,
        exp_data_aug_format=False,
        exp_build_bibl=False,
        **kwargs,
    ):
        super().__init__()
        ...
        if exp_build_bibl:
            assert "[GEN]" in self.tokenizer.get_vocab() and "[ANS]" in self.tokenizer.get_vocab()
    ...
    def read_file(self, main_file, url_patch=None, raw_graphs=None, data_root=None):
        ...
        for i, (line, url, graph) in enumerate(zip(f, f_url, f_graph)):
            if self.hparams.exp_data_aug_format and "train" in main_file:
                _, sids, tids, variables = line.split("\t", 3)
            else:
    ...
    def train_dataloader(self):
        loader = DataLoader(
            ...
            collate_fn=partial(self.collator, split="train"),
    ...
    def val_dataloader(self):
        loader = DataLoader(
            ...
            num_workers=self.hparams.num_workers,
            pin_memory=self.hparams.pin_memory,
            collate_fn=partial(self.collator, split="dev"),
    ...
    def test_dataloader(self):
        loader = DataLoader(
            ...
            num_workers=self.hparams.num_workers,
            pin_memory=self.hparams.pin_memory,
            collate_fn=partial(self.collator, split="test"),
            **self.get_sampler("test"),
        )
        log.info(f"Test dataloader: {len(loader)}")
        return loader

    def collator(self, data, split):
        batch = self._collator(data, split=split)
        if self.hparams.exp_build_bibl and split == "train":
            gen_token = self.tokenizer.convert_tokens_to_ids("[GEN]")
            sep_token = self.tokenizer.convert_tokens_to_ids("[ANS]")
            mask_token = self.tokenizer.convert_tokens_to_ids("<mask>")
            gen_data = []
            rec_data = []
            for item in data:
                src, tgt, id_ = item["src"], item["tgt"], item["id"]
                gen_item = {
                    "id": id_,
                    "src": src[:1] + [gen_token] + src[-1:],
                    "tgt": src[:-1] + [sep_token] + tgt[1:],
                    "var": item["var"],
                }
                gen_data.append(gen_item)

                rec_src = src[:-1] + [sep_token] + tgt[1:]
                random_mask = [random() < 0.5 for _ in range(len(rec_src))]
                random_mask[0] = random_mask[-1] = random_mask[len(src) - 1] = False
                rec_src = [mask_token if flag else token for token, flag in zip(rec_src, random_mask)]
                rec_item = {"id": id_, "src": rec_src, "tgt": src[:-1] + [sep_token] + tgt[1:], "var": item["var"]}
                rec_data.append(rec_item)

            gen_batch = self._collator(gen_data, split=split)
            rec_batch = self._collator(rec_data, split=split)
            del gen_batch["id"], rec_batch["id"]

            batch["gen_batch"] = gen_batch
            batch["rec_batch"] = rec_batch
        return batch

    def _collator(self, data, split):
        ...
        for tgt_item in tgt:
            ...
            processed_data.append(
                (
                    chunk.inputs[:_len],
                    chunk.inputs_ttypes[:_len],
                    chunk.labels[:_len],
                    chunk.labels_ttypes[:_len],
        ...
        if self.hparams.word_mask > 0 and split == 'train':
            src_ = []
            mask_token = self.tokenizer.convert_tokens_to_ids("<mask>")
            for item in src:
                random_mask = [random() < self.hparams.word_mask for _ in range(len(item))]
                random_mask[0] = random_mask[-1] = False
                item = [mask_token if flag else token for token, flag in zip(item, random_mask)]
                src_.append(item)
            src = src_
        ...
        # )
        batch["decoder_attention_mask"] = pad(
            [item[4] for item in processed_data], 0, self.hparams.pad_to_multiple_of
        )
...
if __name__ == "__main__":
    datamodule = Text2GraphDataModule(
        "data/AMR/bart",
        "amrtoken-dvar/train.tsv",
        "amrtoken-dvar/dev.tsv",
        "amrtoken-dvar/dev.graph",
        "amrtoken-dvar/test.tsv",
        "amrtoken-dvar/test.graph",
        tokenizer_path="data/AMR/bart/model-base-amrtoken",
        maskrules=dict(name="txl", sequence_length=256, memory_length=256),
        batch_size=4,
    ...
    for i, batch in enumerate(datamodule.train_dataloader()):
        if i < 5:
            ...
            print("pointer:")
            for i, p in enumerate(batch["tgt_pt_pointer"][0].tolist()):
                if p != -100:
                    print(tokenizer.convert_ids_to_tokens(batch["decoder_input_ids"][0, [i + 1, p]]), i, p)
    ...
    #     tokenizer_path="data/AMR/bart/model-base-amrtoken",
    #     maskrules=dict(
    #         name="stack_compose_double_closing_nt",
    #         sequence_length=256,
    #         memory_length=256,
    #         use_different_attn_fns=False,
