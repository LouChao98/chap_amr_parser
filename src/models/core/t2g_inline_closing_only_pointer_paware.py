...
from src.models.components.extended_transformers import (
    bart_adapter_closing_only_pointer_paware,
)
...
class Text2GraphInline(nn.Module):
    supported_models = {
        "bart_paware_adapter": bart_adapter_closing_only_pointer_paware.BartForConditionalGeneration,
    }
    ...
    def forward(self, inputs):
        ...
        pointer_ids = torch.full_like(inputs["closing_pointers"], -100)
        pointer_ids[:, 1:] = inputs["closing_pointers"][:, :-1]
...
class BilinearAligner(nn.Module):
    def __init__(self, input_size, hidden_size, bart_layers, deeper, fence) -> None:
        super().__init__()
    ...
    def forward(self, model_output, pointer_mask):
        ...
        if self.fence:
            dec_h1, dec_h2 = torch.chunk(dec_h, 2, -1)
            dec_h2 = torch.cat([dec_h2[:, 1:], dec_h2.new_zeros((len(dec_h2), 1, dec_h2.shape[-1]))], dim=1)
            dec_h = torch.cat([dec_h1, dec_h2], 2)
