...
class BartDecoder(BartPretrainedModel):
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        tg_heads = config.tg_args["tg_heads"]
        if tg_heads == "all":
            tg_heads = [1] * self.config.decoder_attention_heads
        else:
            tg_heads = set(tg_heads)
            tg_heads = [int(i in tg_heads) for i in range(self.config.decoder_attention_heads)]
        self.register_buffer("attention_mask_on_heads", torch.tensor(tg_heads))

        tg_layers = config.tg_args["tg_layers"]
        if tg_layers == "all":
            self.tg_layers = set(range(self.config.decoder_layers))
        else:
            self.tg_layers = set(tg_layers)
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        if attention_mask is not None:
            ...
            expanded_attn_mask = expanded_attn_mask * self.attention_mask_on_heads.view(1, -1, 1, 1)
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            attention_mask = tg_attention_mask if idx in self.tg_layers else causal_attention_mask
