    ...
    def forward(
        ...
        if self.paware_mode != "disable":
            ...
            if self.pointer_net.is_generating and self.pointer_net.generation_hidden_states_cache_l0 is not None:
                ...
                tgt_pointed = torch.nn.functional.dropout(tgt_pointed, p=self.config.dropout, training=self.training)
        ...
        if pointer_labels is not None:
            predicted_pointers = self.pointer_net(outputs, decoder_inputs_embeds, pointer_mask)
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            decoder = self.get_decoder()
            token_emb = decoder.embed_tokens(next_tokens.unsqueeze(-1)) * decoder.embed_scale
            next_pointer_logits = self.pointer_net(outputs, token_emb)[:, 0]
            next_pointer_logits[~is_pointable] = torch.finfo(next_pointer_logits.dtype).min
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # ** x selected tokens x pointer positions
            decoder = self.get_decoder()
            token_emb = decoder.embed_tokens(selected_tokens) * decoder.embed_scale
            next_pointer_logits = self.pointer_net(outputs, token_emb)
            ...
            next_pointer_scores = None  # suppress flake8
            raise NotImplementedError
            if (pp := pruning_pointer) > 0 and pp < next_pointer_scores.shape[3]:
                next_pointer_scores, selected_pointers = torch.topk(
                    next_pointer_scores, pp, dim=3, largest=True, sorted=False
                )
            else:
                pp = next_pointer_scores.shape[3]
            ...
            next_pointer_scores = next_pointer_scores.transpose(2, 3).contiguous()

            next_pointer_scores.masked_fill_(~can_point_here.unsqueeze(1), torch.finfo(next_pointer_logits.dtype).min)
