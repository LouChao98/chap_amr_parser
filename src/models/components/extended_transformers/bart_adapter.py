...
class BartAttention(OrigBartAttention):
    def __init__(
        ...
        self.new_mask_impl = new_mask_impl
        ...
        if self.use_rpe:
            ...
            if rpe_type == "simple":
                ...
                self.position_embedding = nn.Parameter(torch.randn(self.num_rpe, num_heads, self.head_dim))
        ...
        if self.mask_strength.startswith("learnable"):
            self.mask_strength_param = nn.Parameter(torch.zeros(self.num_heads))
        elif self.mask_strength == "step":
            self.register_buffer("mask_strength_param", -torch.tensor(AliBi(self.num_heads).slopes * 5))
    ...
    def forward(
        ...
        if self.use_rpe:
            if self.rpe_type == "simple":
                _q = query_states.view(bsz, self.num_heads, -1, self.head_dim)  # bhix
                q2 = torch.einsum("bhix,nhx->bhin", _q, self.position_embedding)
                rpe_indices = relative_distance.clamp(self.min_rpe, self.max_rpe) - self.min_rpe  # bij
                rpe_indices = rpe_indices.unsqueeze(1).expand(-1, self.num_heads, -1, -1)
                attn_weight2 = q2.gather(3, rpe_indices)
                attn_weights = attn_weights + attn_weight2.view(bsz * self.num_heads, tgt_len, src_len)
        ...
        if attention_mask is not None:
            if self.adjust_entropy:
                coeff = (attention_mask > -10).sum(-1, keepdim=True).clamp(1)
                coeff = coeff.log() / math.log(128)
                attn_weights = (attn_weights.view(bsz, self.num_heads, tgt_len, src_len) * coeff).view(
                    -1, tgt_len, src_len
                )
            if self.mask_strength != "none":
                param = self.get_mask_strength()
                tg_mask = (attention_mask < -10) * param[None, :, None, None]
                if has_past:
                    attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len) + tg_mask
                else:
                    causal_mask = attn_weights.new_full((tgt_len, src_len), float("-inf")).triu_(1)[None, None, :, :]
                    attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len) + causal_mask + tg_mask
            else:
                if not self.new_mask_impl:
                    attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len) + attention_mask
                else:
                    attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)
                    attn_weights[(attention_mask < -10).expand_as(attn_weights)] = torch.finfo(attn_weights.dtype).min
            attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
    ...
    def get_mask_strength(self):
        if self.mask_strength.startswith("learnable"):
            if "sigmoid" in self.mask_strength:
                param = -100 * self.mask_strength_param.sigmoid()
            elif "leakyrelue" in self.mask_strength:
                param = -100 * F.leaky_relu(self.mask_strength)
            else:
                param = -100 * self.mask_strength_param
        elif self.mask_strength == "step":
            param = self.mask_strength_param
        return param
...
class BartDecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.tg_add_dropout = config.tg_args["tg_add_dropout"]
        ...
        if self.tg_bypass_impl > 0:
            ...
            self.tg_layer_dropout = config.tg_args["tg_layer_dropout"]
            ...
            if (proj := config.tg_args["tg_bypass_proj_size"]) is not None:
                ...
                embed_dim = proj
                self.scale = (512 / proj) if config.tg_args["tg_bypass_proj_scale"] else 1
            else:
            ...
            self.tg_self_attn = BartAttention(
                ...
                dropout=config.tg_args["tg_attention_dropout"],
                is_decoder=True,
                new_mask_impl=config.tg_args["tg_new_mask_impl"],
                adjust_entropy=config.tg_args["tg_adjust_entropy"],
                mask_strength=config.tg_args["tg_mask_strength"],
                **rpe_args,
            )
            ...
            if self.tg_bypass_impl >= 4:
                self.tg_gating = nn.Sequential(
                    nn.Linear(self.embed_dim, self.embed_dim),
                    nn.Sigmoid() if config.tg_args["tg_gating_use_sigmoid"] else ReLU2(),
                )
                self.tg_gating_use_raw_inp = config.tg_args["tg_gating_use_raw_inp"]
    ...
    def forward(
        ...
        if self.tg_bypass_impl > 0:
            ...
            if self.tg_add_dropout:
                hidden_states2 = nn.functional.dropout(hidden_states2, p=self.dropout, training=self.training)
            hidden_states2 = hidden_states2 * self.scale
            if self.tg_bypass_impl >= 4:
                gating = self.tg_gating(residual if self.tg_gating_use_raw_inp else hidden_states1)
                hidden_states2 = gating * hidden_states2
            if self.training and random.random() < self.tg_layer_dropout:
                hidden_states = residual + 2 * hidden_states2
            else:
        ...
        if encoder_hidden_states is not None:
            ...
            if self.use_external_feature:
                ...
                (
                    hidden_states2,
                    cross_attn_weights2,
                    cross_attn_present_key_value2,
                ) = self.external_feature_attn(
                    hidden_states=hidden_states,
                ...
                cross_attn_present_key_value = tuple(
                    zip(cross_attn_present_key_value1, cross_attn_present_key_value2)
                )
            else:
                (
                    hidden_states,
        ...
        if output_attentions:
            if self.tg_debug_mode and self.tg_bypass_impl > 0:
                outputs += (
                    (self_attn_weights1, self_attn_weights2),
                    cross_attn_weights,
                )
            elif self.return_bypass_attn and self.tg_bypass_impl > 0:
                outputs += (
                    self_attn_weights2,
                    cross_attn_weights,
                )
...
class BartForConditionalGeneration(OrigBartForConditionalGeneration):
    ...
    default_tg_args = {
        ...
        "tg_debug_mode": False,
        "tg_multi_mask": False,
        "tg_gating_use_raw_inp": True,
        "tg_gating_use_sigmoid": True,
        "tg_new_mask_impl": False,
        "tg_adjust_entropy": False,
        "rpe_init": "none",
        "tg_mask_strength": "none",
        "tg_fix_init": False,
        "tg_layer_dropout": 0.0,
        "tg_attention_dropout": 0.1,
        "tg_add_dropout": True,
    }
    ...
    def __init__(self, config: BartConfig, maskagent, tg_args):
        ...
        for layer in self.get_decoder().layers:
            if (
                getattr(layer, "tg_bypass_impl", 0) > 0
                and hasattr(layer.tg_self_attn, "position_embedding")
                and layer.tg_self_attn.rpe_type == "simple"
            ):
                if config.tg_args["rpe_init"] == 0:
                    torch.nn.init.zeros_(layer.tg_self_attn.position_embedding.data)
                elif isinstance(config.tg_args["rpe_init"], float):
                    torch.nn.init.normal_(layer.tg_self_attn.position_embedding.data, config.tg_args["rpe_init"])
        ...
        **kwargs,
        ...
        if decoder_input_ids.shape[1] == 1:
            if self.maskagent.is_double_closing:
                ...
                raise ValueError(f"bad maskrules: {self.maskagent.maskrules_name}")
